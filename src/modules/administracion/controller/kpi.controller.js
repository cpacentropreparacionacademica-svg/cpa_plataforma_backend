...
const logger = rootLogger.child({ module: "kpi.controller" });
const MODULE_BASE_NAME = "Kpi.controller";
const ID_PARAM_NAMES = ['id_kpi'];
...
async function create(req, res) {
  ...
  const action = "kpi_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await KpiService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  ...
  try {
    ...
    const result = await KpiService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await KpiService.get(idOrParams);
