const EmpleadoPosicionPagoService = require("../service/empleado_posicion_pago.service");
...
const logger = rootLogger.child({ module: "empleado_posicion_pago.controller" });
const MODULE_BASE_NAME = "EmpleadoPosicionPago.controller";
const ID_PARAM_NAMES = ['id_empleado_posicion'];
...
async function create(req, res) {
  ...
  const action = "empleado_posicion_pago_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await EmpleadoPosicionPagoService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "empleado_posicion_pago_update";
  ...
  try {
    ...
    const result = await EmpleadoPosicionPagoService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  const startedAt = Date.now();
  const action = "empleado_posicion_pago_get";
  ...
  try {
    ...
    const result = await EmpleadoPosicionPagoService.get(idOrParams);
...
async function list(req, res) {
  const startedAt = Date.now();
  const action = "empleado_posicion_pago_list";
  const moduleName = `${MODULE_BASE_NAME}.list`;

  try {
    sendAttemptingRequest(req, logger, action, { query: req.query });

    const result = await EmpleadoPosicionPagoService.list(req.query);
