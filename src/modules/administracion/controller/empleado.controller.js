...
const logger = rootLogger.child({ module: "empleado.controller" });
const MODULE_BASE_NAME = "Empleado.controller";
const ID_PARAM_NAMES = ['id_empleado'];
...
async function create(req, res) {
  ...
  const action = "empleado_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await EmpleadoService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  ...
  try {
    ...
    const result = await EmpleadoService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await EmpleadoService.get(idOrParams);
