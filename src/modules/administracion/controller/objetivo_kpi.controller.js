const ObjetivoKpiService = require("../service/objetivo_kpi.service");
...
const logger = rootLogger.child({ module: "objetivo_kpi.controller" });
const MODULE_BASE_NAME = "ObjetivoKpi.controller";
const ID_PARAM_NAMES = ['id_objetivo_kpi'];
...
async function create(req, res) {
  ...
  const action = "objetivo_kpi_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await ObjetivoKpiService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "objetivo_kpi_update";
  ...
  try {
    ...
    const result = await ObjetivoKpiService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await ObjetivoKpiService.get(idOrParams);
