const PosicionService = require("../service/posicion.service");
...
const logger = rootLogger.child({ module: "posicion.controller" });
const MODULE_BASE_NAME = "Posicion.controller";
const ID_PARAM_NAMES = ['id_posicion'];
...
async function create(req, res) {
  ...
  const action = "posicion_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await PosicionService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  ...
  try {
    ...
    const result = await PosicionService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await PosicionService.get(idOrParams);
