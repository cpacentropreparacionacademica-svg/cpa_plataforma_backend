const ActionLogService = require("../service/action_log.service");
...
const logger = rootLogger.child({ module: "action_log.controller" });
const MODULE_BASE_NAME = "ActionLog.controller";
const ID_PARAM_NAMES = ['id_action'];
...
async function create(req, res) {
  ...
  const action = "action_log_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await ActionLogService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "action_log_update";
  ...
  try {
    ...
    const result = await ActionLogService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await ActionLogService.get(idOrParams);
