...
const logger = rootLogger.child({ module: "permiso.controller" });
const MODULE_BASE_NAME = "Permiso.controller";
const ID_PARAM_NAMES = ['id_permiso'];
...
async function create(req, res) {
  ...
  const action = "permiso_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await PermisoService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  ...
  try {
    ...
    const result = await PermisoService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await PermisoService.get(idOrParams);
