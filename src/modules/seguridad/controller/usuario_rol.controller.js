const UsuarioRolService = require("../service/usuario_rol.service");
...
const logger = rootLogger.child({ module: "usuario_rol.controller" });
const MODULE_BASE_NAME = "UsuarioRol.controller";
const ID_PARAM_NAMES = ['id_persona', 'id_rol'];
...
async function create(req, res) {
  ...
  const action = "usuario_rol_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await UsuarioRolService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "usuario_rol_update";
  ...
  try {
    ...
    const result = await UsuarioRolService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await UsuarioRolService.get(idOrParams);
