...
const logger = rootLogger.child({ module: "rol.controller" });
const MODULE_BASE_NAME = "Rol.controller";
const ID_PARAM_NAMES = ['id_rol'];
...
async function create(req, res) {
  ...
  const action = "rol_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await RolService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  ...
  try {
    ...
    const result = await RolService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await RolService.get(idOrParams);
