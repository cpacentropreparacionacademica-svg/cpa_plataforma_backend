const UsuarioPermisoService = require("../service/usuario_permiso.service");
...
const logger = rootLogger.child({ module: "usuario_permiso.controller" });
const MODULE_BASE_NAME = "UsuarioPermiso.controller";
const ID_PARAM_NAMES = ['id_persona', 'id_permiso'];
...
async function create(req, res) {
  ...
  const action = "usuario_permiso_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await UsuarioPermisoService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "usuario_permiso_update";
  ...
  try {
    ...
    const result = await UsuarioPermisoService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  const startedAt = Date.now();
  const action = "usuario_permiso_get";
  ...
  try {
    ...
    const result = await UsuarioPermisoService.get(idOrParams);
