const RolPermisoService = require("../service/rol_permiso.service");
...
const logger = rootLogger.child({ module: "rol_permiso.controller" });
const MODULE_BASE_NAME = "RolPermiso.controller";
const ID_PARAM_NAMES = ['id_rol', 'id_permiso'];
...
async function create(req, res) {
  ...
  const action = "rol_permiso_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await RolPermisoService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "rol_permiso_update";
  ...
  try {
    ...
    const result = await RolPermisoService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await RolPermisoService.get(idOrParams);
