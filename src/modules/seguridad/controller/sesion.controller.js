...
const logger = rootLogger.child({ module: "sesion.controller" });
const MODULE_BASE_NAME = "Sesion.controller";
const ID_PARAM_NAMES = ['id_sesion'];
...
async function create(req, res) {
  ...
  const action = "sesion_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await SesionService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  ...
  try {
    ...
    const result = await SesionService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await SesionService.get(idOrParams);
