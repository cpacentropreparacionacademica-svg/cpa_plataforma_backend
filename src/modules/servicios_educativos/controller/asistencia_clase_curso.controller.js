const AsistenciaClaseCursoService = require("../service/asistencia_clase_curso.service");
...
const logger = rootLogger.child({ module: "asistencia_clase_curso.controller" });
const MODULE_BASE_NAME = "AsistenciaClaseCurso.controller";
const ID_PARAM_NAMES = ['id_asistencia'];
...
async function create(req, res) {
  ...
  const action = "asistencia_clase_curso_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await AsistenciaClaseCursoService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "asistencia_clase_curso_update";
  ...
  try {
    ...
    const result = await AsistenciaClaseCursoService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  const startedAt = Date.now();
  const action = "asistencia_clase_curso_get";
  ...
  try {
    ...
    const result = await AsistenciaClaseCursoService.get(idOrParams);
...
async function list(req, res) {
  const startedAt = Date.now();
  const action = "asistencia_clase_curso_list";
  const moduleName = `${MODULE_BASE_NAME}.list`;

  try {
    sendAttemptingRequest(req, logger, action, { query: req.query });

    const result = await AsistenciaClaseCursoService.list(req.query);
