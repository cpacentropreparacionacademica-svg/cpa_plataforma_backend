const ClasePorHoraService = require("../service/clase_por_hora.service");
...
const logger = rootLogger.child({ module: "clase_por_hora.controller" });
const MODULE_BASE_NAME = "ClasePorHora.controller";
const ID_PARAM_NAMES = ['id_clase'];
...
async function create(req, res) {
  ...
  const action = "clase_por_hora_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await ClasePorHoraService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "clase_por_hora_update";
  ...
  try {
    ...
    const result = await ClasePorHoraService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  const startedAt = Date.now();
  const action = "clase_por_hora_get";
  ...
  try {
    ...
    const result = await ClasePorHoraService.get(idOrParams);
