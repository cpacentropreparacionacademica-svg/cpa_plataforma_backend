const PaquetesProductoEducativoService = require("../service/paquetes_producto_educativo.service");
...
const logger = rootLogger.child({ module: "paquetes_producto_educativo.controller" });
const MODULE_BASE_NAME = "PaquetesProductoEducativo.controller";
const ID_PARAM_NAMES = ['id_paquete'];
...
async function create(req, res) {
  ...
  const action = "paquetes_producto_educativo_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await PaquetesProductoEducativoService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "paquetes_producto_educativo_update";
  ...
  try {
    ...
    const result = await PaquetesProductoEducativoService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  const startedAt = Date.now();
  const action = "paquetes_producto_educativo_get";
  ...
  try {
    ...
    const result = await PaquetesProductoEducativoService.get(idOrParams);
...
async function list(req, res) {
  const startedAt = Date.now();
  const action = "paquetes_producto_educativo_list";
  const moduleName = `${MODULE_BASE_NAME}.list`;

  try {
    sendAttemptingRequest(req, logger, action, { query: req.query });

    const result = await PaquetesProductoEducativoService.list(req.query);
