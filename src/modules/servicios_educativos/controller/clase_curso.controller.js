const ClaseCursoService = require("../service/clase_curso.service");
...
const logger = rootLogger.child({ module: "clase_curso.controller" });
const MODULE_BASE_NAME = "ClaseCurso.controller";
const ID_PARAM_NAMES = ['id_clase_curso'];
...
async function create(req, res) {
  ...
  const action = "clase_curso_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await ClaseCursoService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "clase_curso_update";
  ...
  try {
    ...
    const result = await ClaseCursoService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await ClaseCursoService.get(idOrParams);
