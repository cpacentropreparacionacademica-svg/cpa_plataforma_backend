const CursoVersionService = require("../service/curso_version.service");
...
const logger = rootLogger.child({ module: "curso_version.controller" });
const MODULE_BASE_NAME = "CursoVersion.controller";
const ID_PARAM_NAMES = ['id_curso_version'];
...
async function create(req, res) {
  ...
  const action = "curso_version_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await CursoVersionService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "curso_version_update";
  ...
  try {
    ...
    const result = await CursoVersionService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  const startedAt = Date.now();
  const action = "curso_version_get";
  ...
  try {
    ...
    const result = await CursoVersionService.get(idOrParams);
