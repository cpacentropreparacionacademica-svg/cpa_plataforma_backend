const ProductoEducativoService = require("../service/producto_educativo.service");
...
const logger = rootLogger.child({ module: "producto_educativo.controller" });
const MODULE_BASE_NAME = "ProductoEducativo.controller";
const ID_PARAM_NAMES = ['id_producto_educativo'];
...
async function create(req, res) {
  ...
  const action = "producto_educativo_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await ProductoEducativoService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "producto_educativo_update";
  ...
  try {
    ...
    const result = await ProductoEducativoService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  const startedAt = Date.now();
  const action = "producto_educativo_get";
  ...
  try {
    ...
    const result = await ProductoEducativoService.get(idOrParams);
...
async function list(req, res) {
  const startedAt = Date.now();
  const action = "producto_educativo_list";
  const moduleName = `${MODULE_BASE_NAME}.list`;

  try {
    sendAttemptingRequest(req, logger, action, { query: req.query });

    const result = await ProductoEducativoService.list(req.query);
