const MateriaTreeService = require("../service/materia_tree.service");
...
const logger = rootLogger.child({ module: "materia_tree.controller" });
const MODULE_BASE_NAME = "MateriaTree.controller";
const ID_PARAM_NAMES = ['id_tree'];
...
async function create(req, res) {
  ...
  const action = "materia_tree_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await MateriaTreeService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "materia_tree_update";
  ...
  try {
    ...
    const result = await MateriaTreeService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await MateriaTreeService.get(idOrParams);
