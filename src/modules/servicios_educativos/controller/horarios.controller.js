...
const logger = rootLogger.child({ module: "horarios.controller" });
const MODULE_BASE_NAME = "Horarios.controller";
const ID_PARAM_NAMES = ['id_horario'];
...
async function create(req, res) {
  ...
  const action = "horarios_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await HorariosService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  ...
  try {
    ...
    const result = await HorariosService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await HorariosService.get(idOrParams);
