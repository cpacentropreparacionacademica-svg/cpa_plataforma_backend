const MovimientoDetalleService = require("../service/movimiento_detalle.service");
...
const logger = rootLogger.child({ module: "movimiento_detalle.controller" });
const MODULE_BASE_NAME = "MovimientoDetalle.controller";
...
async function create(req, res) {
  ...
  const action = "movimiento_detalle_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await MovimientoDetalleService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "movimiento_detalle_update";
  ...
  try {
    ...
    const result = await MovimientoDetalleService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  const startedAt = Date.now();
  const action = "movimiento_detalle_get";
  ...
  try {
    ...
    const result = await MovimientoDetalleService.get(idOrParams);
...
async function list(req, res) {
  const startedAt = Date.now();
  const action = "movimiento_detalle_list";
  const moduleName = `${MODULE_BASE_NAME}.list`;

  try {
    sendAttemptingRequest(req, logger, action, { query: req.query });

    const result = await MovimientoDetalleService.list(req.query);
