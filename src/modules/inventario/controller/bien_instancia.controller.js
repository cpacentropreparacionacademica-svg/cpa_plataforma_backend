const BienInstanciaService = require("../service/bien_instancia.service");
...
const logger = rootLogger.child({ module: "bien_instancia.controller" });
const MODULE_BASE_NAME = "BienInstancia.controller";
const ID_PARAM_NAMES = ['id_bien_instancia'];
...
async function create(req, res) {
  ...
  const action = "bien_instancia_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await BienInstanciaService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "bien_instancia_update";
  ...
  try {
    ...
    const result = await BienInstanciaService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  const startedAt = Date.now();
  const action = "bien_instancia_get";
  ...
  try {
    ...
    const result = await BienInstanciaService.get(idOrParams);
