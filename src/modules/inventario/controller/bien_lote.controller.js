const BienLoteService = require("../service/bien_lote.service");
...
const logger = rootLogger.child({ module: "bien_lote.controller" });
const MODULE_BASE_NAME = "BienLote.controller";
const ID_PARAM_NAMES = ['id_lote'];
...
async function create(req, res) {
  ...
  const action = "bien_lote_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await BienLoteService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  ...
  try {
    ...
    const result = await BienLoteService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await BienLoteService.get(idOrParams);
