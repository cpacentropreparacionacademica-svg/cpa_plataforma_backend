const BienService = require("../service/bien.service");
...
const logger = rootLogger.child({ module: "bien.controller" });
const MODULE_BASE_NAME = "Bien.controller";
const ID_PARAM_NAMES = ['id_bien'];
...
async function create(req, res) {
  ...
  const action = "bien_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await BienService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  ...
  try {
    ...
    const result = await BienService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await BienService.get(idOrParams);
