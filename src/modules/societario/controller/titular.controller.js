const TitularService = require("../service/titular.service");
...
const logger = rootLogger.child({ module: "titular.controller" });
const MODULE_BASE_NAME = "Titular.controller";
const ID_PARAM_NAMES = ['id_titular'];
...
async function create(req, res) {
  ...
  const action = "titular_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await TitularService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  ...
  try {
    ...
    const result = await TitularService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await TitularService.get(idOrParams);
