const DividendoPagoService = require("../service/dividendo_pago.service");
...
const logger = rootLogger.child({ module: "dividendo_pago.controller" });
const MODULE_BASE_NAME = "DividendoPago.controller";
const ID_PARAM_NAMES = ['id_dividendo_pago'];
...
async function create(req, res) {
  ...
  const action = "dividendo_pago_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await DividendoPagoService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "dividendo_pago_update";
  ...
  try {
    ...
    const result = await DividendoPagoService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  const startedAt = Date.now();
  const action = "dividendo_pago_get";
  ...
  try {
    ...
    const result = await DividendoPagoService.get(idOrParams);
