const TransferenciaTituloService = require("../service/transferencia_titulo.service");
...
const logger = rootLogger.child({ module: "transferencia_titulo.controller" });
const MODULE_BASE_NAME = "TransferenciaTitulo.controller";
const ID_PARAM_NAMES = ['id_transferencia'];
...
async function create(req, res) {
  ...
  const action = "transferencia_titulo_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await TransferenciaTituloService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "transferencia_titulo_update";
  ...
  try {
    ...
    const result = await TransferenciaTituloService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  const startedAt = Date.now();
  const action = "transferencia_titulo_get";
  ...
  try {
    ...
    const result = await TransferenciaTituloService.get(idOrParams);
...
async function list(req, res) {
  const startedAt = Date.now();
  const action = "transferencia_titulo_list";
  const moduleName = `${MODULE_BASE_NAME}.list`;

  try {
    sendAttemptingRequest(req, logger, action, { query: req.query });

    const result = await TransferenciaTituloService.list(req.query);
