const ClaseTituloService = require("../service/clase_titulo.service");
...
const logger = rootLogger.child({ module: "clase_titulo.controller" });
const MODULE_BASE_NAME = "ClaseTitulo.controller";
const ID_PARAM_NAMES = ['id_clase_titulo'];
...
async function create(req, res) {
  ...
  const action = "clase_titulo_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await ClaseTituloService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "clase_titulo_update";
  ...
  try {
    ...
    const result = await ClaseTituloService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await ClaseTituloService.get(idOrParams);
