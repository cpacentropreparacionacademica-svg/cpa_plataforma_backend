const TenenciaService = require("../service/tenencia.service");
...
const logger = rootLogger.child({ module: "tenencia.controller" });
const MODULE_BASE_NAME = "Tenencia.controller";
const ID_PARAM_NAMES = ['id_tenencia'];
...
async function create(req, res) {
  ...
  const action = "tenencia_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await TenenciaService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  ...
  try {
    ...
    const result = await TenenciaService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await TenenciaService.get(idOrParams);
