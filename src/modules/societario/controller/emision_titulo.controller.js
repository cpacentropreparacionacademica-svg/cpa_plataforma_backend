const EmisionTituloService = require("../service/emision_titulo.service");
...
const logger = rootLogger.child({ module: "emision_titulo.controller" });
const MODULE_BASE_NAME = "EmisionTitulo.controller";
const ID_PARAM_NAMES = ['id_emision'];
...
async function create(req, res) {
  ...
  const action = "emision_titulo_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await EmisionTituloService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "emision_titulo_update";
  ...
  try {
    ...
    const result = await EmisionTituloService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  const startedAt = Date.now();
  const action = "emision_titulo_get";
  ...
  try {
    ...
    const result = await EmisionTituloService.get(idOrParams);
