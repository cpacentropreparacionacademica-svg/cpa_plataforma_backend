const EdificioService = require("../service/edificio.service");
...
const logger = rootLogger.child({ module: "edificio.controller" });
const MODULE_BASE_NAME = "Edificio.controller";
const ID_PARAM_NAMES = ['id_edificio'];
...
async function create(req, res) {
  ...
  const action = "edificio_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await EdificioService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  ...
  try {
    ...
    const result = await EdificioService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await EdificioService.get(idOrParams);
