const SucursalService = require("../service/sucursal.service");
...
const logger = rootLogger.child({ module: "sucursal.controller" });
const MODULE_BASE_NAME = "Sucursal.controller";
const ID_PARAM_NAMES = ['id_sucursal'];
...
async function create(req, res) {
  ...
  const action = "sucursal_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await SucursalService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  ...
  try {
    ...
    const result = await SucursalService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await SucursalService.get(idOrParams);
