const EncargadoService = require("../service/encargado.service");
...
const logger = rootLogger.child({ module: "encargado.controller" });
const MODULE_BASE_NAME = "Encargado.controller";
const ID_PARAM_NAMES = ['id_asignacion'];
...
async function create(req, res) {
  ...
  const action = "encargado_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await EncargadoService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  ...
  try {
    ...
    const result = await EncargadoService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await EncargadoService.get(idOrParams);
