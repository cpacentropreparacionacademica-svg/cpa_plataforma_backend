const TiendaService = require("../service/tienda.service");
...
const logger = rootLogger.child({ module: "tienda.controller" });
const MODULE_BASE_NAME = "Tienda.controller";
const ID_PARAM_NAMES = ['id_tienda'];
...
async function create(req, res) {
  ...
  const action = "tienda_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await TiendaService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  ...
  try {
    ...
    const result = await TiendaService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await TiendaService.get(idOrParams);
