const EspacioService = require("../service/espacio.service");
...
const logger = rootLogger.child({ module: "espacio.controller" });
const MODULE_BASE_NAME = "Espacio.controller";
const ID_PARAM_NAMES = ['id_espacio'];
...
async function create(req, res) {
  ...
  const action = "espacio_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await EspacioService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  ...
  try {
    ...
    const result = await EspacioService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await EspacioService.get(idOrParams);
