...
const logger = rootLogger.child({ module: "pago.controller" });
const MODULE_BASE_NAME = "Pago.controller";
...
async function create(req, res) {
  ...
  const action = "pago_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await PagoService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  ...
  try {
    ...
    const result = await PagoService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await PagoService.get(idOrParams);
