const DeudaService = require("../service/deuda.service");
...
const logger = rootLogger.child({ module: "deuda.controller" });
const MODULE_BASE_NAME = "Deuda.controller";
const ID_PARAM_NAMES = ['id_deuda'];
...
async function create(req, res) {
  ...
  const action = "deuda_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await DeudaService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  ...
  try {
    ...
    const result = await DeudaService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await DeudaService.get(idOrParams);
