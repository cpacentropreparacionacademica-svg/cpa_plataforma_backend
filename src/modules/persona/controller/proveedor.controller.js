...
const logger = rootLogger.child({ module: "proveedor.controller" });
const MODULE_BASE_NAME = "Proveedor.controller";
const ID_PARAM_NAMES = ['id_proveedor'];
...
async function create(req, res) {
  ...
  const action = "proveedor_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await ProveedorService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  ...
  try {
    ...
    const result = await ProveedorService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await ProveedorService.get(idOrParams);
