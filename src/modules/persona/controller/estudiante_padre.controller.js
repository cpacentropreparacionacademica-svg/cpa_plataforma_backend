const EstudiantePadreService = require("../service/estudiante_padre.service");
...
const logger = rootLogger.child({ module: "estudiante_padre.controller" });
const MODULE_BASE_NAME = "EstudiantePadre.controller";
const ID_PARAM_NAMES = ['id_asociacion'];
...
async function create(req, res) {
  ...
  const action = "estudiante_padre_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await EstudiantePadreService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "estudiante_padre_update";
  ...
  try {
    ...
    const result = await EstudiantePadreService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  const startedAt = Date.now();
  const action = "estudiante_padre_get";
  ...
  try {
    ...
    const result = await EstudiantePadreService.get(idOrParams);
