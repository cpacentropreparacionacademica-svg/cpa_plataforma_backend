const PersonaEstudianteService = require("../service/persona_estudiante.service");
...
const logger = rootLogger.child({ module: "persona_estudiante.controller" });
const MODULE_BASE_NAME = "PersonaEstudiante.controller";
const ID_PARAM_NAMES = ['id_persona'];
...
async function create(req, res) {
  ...
  const action = "persona_estudiante_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await PersonaEstudianteService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "persona_estudiante_update";
  ...
  try {
    ...
    const result = await PersonaEstudianteService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  const startedAt = Date.now();
  const action = "persona_estudiante_get";
  ...
  try {
    ...
    const result = await PersonaEstudianteService.get(idOrParams);
...
async function list(req, res) {
  const startedAt = Date.now();
  const action = "persona_estudiante_list";
  const moduleName = `${MODULE_BASE_NAME}.list`;

  try {
    sendAttemptingRequest(req, logger, action, { query: req.query });

    const result = await PersonaEstudianteService.list(req.query);
