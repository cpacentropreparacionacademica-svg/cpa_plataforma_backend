const PersonaPadreService = require("../service/persona_padre.service");
...
const logger = rootLogger.child({ module: "persona_padre.controller" });
const MODULE_BASE_NAME = "PersonaPadre.controller";
const ID_PARAM_NAMES = ['id_padre'];
...
async function create(req, res) {
  ...
  const action = "persona_padre_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await PersonaPadreService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "persona_padre_update";
  ...
  try {
    ...
    const result = await PersonaPadreService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  const startedAt = Date.now();
  const action = "persona_padre_get";
  ...
  try {
    ...
    const result = await PersonaPadreService.get(idOrParams);
