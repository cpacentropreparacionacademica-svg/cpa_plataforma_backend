...
const logger = rootLogger.child({ module: "persona.controller" });
const MODULE_BASE_NAME = "Persona.controller";
...
async function create(req, res) {
  ...
  const action = "persona_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await PersonaService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  ...
  try {
    ...
    const result = await PersonaService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await PersonaService.get(idOrParams);
