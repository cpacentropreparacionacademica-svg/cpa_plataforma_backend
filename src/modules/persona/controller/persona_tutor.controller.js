const PersonaTutorService = require("../service/persona_tutor.service");
...
const logger = rootLogger.child({ module: "persona_tutor.controller" });
const MODULE_BASE_NAME = "PersonaTutor.controller";
const ID_PARAM_NAMES = ['id_tutor'];
...
async function create(req, res) {
  ...
  const action = "persona_tutor_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await PersonaTutorService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "persona_tutor_update";
  ...
  try {
    ...
    const result = await PersonaTutorService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  const startedAt = Date.now();
  const action = "persona_tutor_get";
  ...
  try {
    ...
    const result = await PersonaTutorService.get(idOrParams);
