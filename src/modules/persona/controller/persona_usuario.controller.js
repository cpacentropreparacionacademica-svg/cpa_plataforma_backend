const PersonaUsuarioService = require("../service/persona_usuario.service");
...
const logger = rootLogger.child({ module: "persona_usuario.controller" });
const MODULE_BASE_NAME = "PersonaUsuario.controller";
...
async function create(req, res) {
  ...
  const action = "persona_usuario_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await PersonaUsuarioService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "persona_usuario_update";
  ...
  try {
    ...
    const result = await PersonaUsuarioService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  const startedAt = Date.now();
  const action = "persona_usuario_get";
  ...
  try {
    ...
    const result = await PersonaUsuarioService.get(idOrParams);
