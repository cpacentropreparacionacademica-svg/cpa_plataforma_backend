const PagoTutorDetalleService = require("../service/pago_tutor_detalle.service");
...
const logger = rootLogger.child({ module: "pago_tutor_detalle.controller" });
const MODULE_BASE_NAME = "PagoTutorDetalle.controller";
const ID_PARAM_NAMES = ['id_pago_tutor_detalle'];
...
async function create(req, res) {
  ...
  const action = "pago_tutor_detalle_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await PagoTutorDetalleService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "pago_tutor_detalle_update";
  ...
  try {
    ...
    const result = await PagoTutorDetalleService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  const startedAt = Date.now();
  const action = "pago_tutor_detalle_get";
  ...
  try {
    ...
    const result = await PagoTutorDetalleService.get(idOrParams);
...
async function list(req, res) {
  const startedAt = Date.now();
  const action = "pago_tutor_detalle_list";
  const moduleName = `${MODULE_BASE_NAME}.list`;

  try {
    sendAttemptingRequest(req, logger, action, { query: req.query });

    const result = await PagoTutorDetalleService.list(req.query);
