const GrupoCuentaService = require("../service/grupo_cuenta.service");
...
const logger = rootLogger.child({ module: "grupo_cuenta.controller" });
const MODULE_BASE_NAME = "GrupoCuenta.controller";
const ID_PARAM_NAMES = ['id_grupo_cuenta'];
...
async function create(req, res) {
  ...
  const action = "grupo_cuenta_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await GrupoCuentaService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "grupo_cuenta_update";
  ...
  try {
    ...
    const result = await GrupoCuentaService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await GrupoCuentaService.get(idOrParams);
