...
const logger = rootLogger.child({ module: "transaccion.controller" });
const MODULE_BASE_NAME = "Transaccion.controller";
const ID_PARAM_NAMES = ['id_transaccion'];
...
async function create(req, res) {
  ...
  const action = "transaccion_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await TransaccionService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "transaccion_update";
  ...
  try {
    ...
    const result = await TransaccionService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await TransaccionService.get(idOrParams);
