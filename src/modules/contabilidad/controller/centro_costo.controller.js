const CentroCostoService = require("../service/centro_costo.service");
...
const logger = rootLogger.child({ module: "centro_costo.controller" });
const MODULE_BASE_NAME = "CentroCosto.controller";
const ID_PARAM_NAMES = ['id_centro_costo'];
...
async function create(req, res) {
  ...
  const action = "centro_costo_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await CentroCostoService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "centro_costo_update";
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await CentroCostoService.get(idOrParams);
