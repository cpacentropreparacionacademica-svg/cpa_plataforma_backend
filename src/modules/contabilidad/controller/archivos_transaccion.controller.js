const ArchivosTransaccionService = require("../service/archivos_transaccion.service");
...
const logger = rootLogger.child({ module: "archivos_transaccion.controller" });
const MODULE_BASE_NAME = "ArchivosTransaccion.controller";
const ID_PARAM_NAMES = ['id_archivo'];
...
async function create(req, res) {
  ...
  const action = "archivos_transaccion_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await ArchivosTransaccionService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "archivos_transaccion_update";
  ...
  try {
    ...
    const result = await ArchivosTransaccionService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  const startedAt = Date.now();
  const action = "archivos_transaccion_get";
  ...
  try {
    ...
    const result = await ArchivosTransaccionService.get(idOrParams);
...
async function list(req, res) {
  const startedAt = Date.now();
  const action = "archivos_transaccion_list";
  const moduleName = `${MODULE_BASE_NAME}.list`;

  try {
    sendAttemptingRequest(req, logger, action, { query: req.query });

    const result = await ArchivosTransaccionService.list(req.query);
