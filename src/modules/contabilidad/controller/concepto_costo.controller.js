const ConceptoCostoService = require("../service/concepto_costo.service");
...
const logger = rootLogger.child({ module: "concepto_costo.controller" });
const MODULE_BASE_NAME = "ConceptoCosto.controller";
const ID_PARAM_NAMES = ['id_concepto'];
...
async function create(req, res) {
  ...
  const action = "concepto_costo_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await ConceptoCostoService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "concepto_costo_update";
  ...
  try {
    ...
    const result = await ConceptoCostoService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  const startedAt = Date.now();
  const action = "concepto_costo_get";
  ...
  try {
    ...
    const result = await ConceptoCostoService.get(idOrParams);
