const CuentaService = require("../service/cuenta.service");
...
const logger = rootLogger.child({ module: "cuenta.controller" });
const MODULE_BASE_NAME = "Cuenta.controller";
const ID_PARAM_NAMES = ['id_cuenta'];
...
async function create(req, res) {
  ...
  const action = "cuenta_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await CuentaService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  ...
  try {
    ...
    const result = await CuentaService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await CuentaService.get(idOrParams);
