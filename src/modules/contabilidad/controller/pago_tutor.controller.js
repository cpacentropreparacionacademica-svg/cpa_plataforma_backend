const PagoTutorService = require("../service/pago_tutor.service");
...
const logger = rootLogger.child({ module: "pago_tutor.controller" });
const MODULE_BASE_NAME = "PagoTutor.controller";
const ID_PARAM_NAMES = ['id_pago_tutor'];
...
async function create(req, res) {
  ...
  const action = "pago_tutor_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await PagoTutorService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "pago_tutor_update";
  ...
  try {
    ...
    const result = await PagoTutorService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  ...
  try {
    ...
    const result = await PagoTutorService.get(idOrParams);
