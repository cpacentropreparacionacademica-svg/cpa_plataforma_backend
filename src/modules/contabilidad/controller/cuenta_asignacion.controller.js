const CuentaAsignacionService = require("../service/cuenta_asignacion.service");
...
const logger = rootLogger.child({ module: "cuenta_asignacion.controller" });
const MODULE_BASE_NAME = "CuentaAsignacion.controller";
const ID_PARAM_NAMES = ['id_cuenta_asignacion'];
...
async function create(req, res) {
  ...
  const action = "cuenta_asignacion_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await CuentaAsignacionService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "cuenta_asignacion_update";
  ...
  try {
    ...
    const result = await CuentaAsignacionService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  const startedAt = Date.now();
  const action = "cuenta_asignacion_get";
  ...
  try {
    ...
    const result = await CuentaAsignacionService.get(idOrParams);
