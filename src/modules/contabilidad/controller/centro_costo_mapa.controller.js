const CentroCostoMapaService = require("../service/centro_costo_mapa.service");
...
const logger = rootLogger.child({ module: "centro_costo_mapa.controller" });
const MODULE_BASE_NAME = "CentroCostoMapa.controller";
const ID_PARAM_NAMES = ['id_cc_mapa'];
...
async function create(req, res) {
  ...
  const action = "centro_costo_mapa_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await CentroCostoMapaService.create(req.body, getAuthUserId(req));
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "centro_costo_mapa_update";
  ...
  try {
    ...
    const result = await CentroCostoMapaService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  const startedAt = Date.now();
  const action = "centro_costo_mapa_get";
  ...
  try {
    ...
    const result = await CentroCostoMapaService.get(idOrParams);
