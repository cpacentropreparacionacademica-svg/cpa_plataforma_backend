const TransaccionMovimientoCuentaService = require("../service/transaccion_movimiento_cuenta.service");
...
const logger = rootLogger.child({ module: "transaccion_movimiento_cuenta.controller" });
const MODULE_BASE_NAME = "TransaccionMovimientoCuenta.controller";
const ID_PARAM_NAMES = ['id_movimiento'];
...
async function create(req, res) {
  ...
  const action = "transaccion_movimiento_cuenta_creation";
  const moduleName = `${MODULE_BASE_NAME}.create`;

  try {
    sendAttemptingRequest(req, logger, action, req.body);

    const result = await TransaccionMovimientoCuentaService.create(req.body, getAuthUserId(req));
    ...
    return res.status(201).json({
      success: true,
      message: result.message || "Movimientos de cuenta registrados correctamente.",
      data: result.data,
    });
  } catch (error) {
...
async function update(req, res) {
  const startedAt = Date.now();
  const action = "transaccion_movimiento_cuenta_update";
  ...
  try {
    ...
    const result = await TransaccionMovimientoCuentaService.update(
      idOrParams,
      req.body,
      getAuthUserId(req)
    );
...
async function get(req, res) {
  const startedAt = Date.now();
  const action = "transaccion_movimiento_cuenta_get";
  ...
  try {
    ...
    const result = await TransaccionMovimientoCuentaService.get(idOrParams);
...
async function list(req, res) {
  const startedAt = Date.now();
  const action = "transaccion_movimiento_cuenta_list";
  const moduleName = `${MODULE_BASE_NAME}.list`;

  try {
    sendAttemptingRequest(req, logger, action, { query: req.query });

    const result = await TransaccionMovimientoCuentaService.list(req.query);
