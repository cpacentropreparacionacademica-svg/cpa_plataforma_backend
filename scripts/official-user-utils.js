...
function getOfficialUserFromEnv() {
  const email = firstEnv('TEST_USER_EMAIL', 'Email', 'EMAIL') || 'pablo.admin@cpa.com';
  const username = firstEnv('TEST_USER_USERNAME', 'Usuario', 'USUARIO', 'USERNAME') || 'pablo.admin';
  const password = firstEnv('TEST_USER_PASSWORD', 'Password', 'PASSWORD') || 'PabloAdmin2026!';
  const idPersona = Number(firstEnv('TEST_USER_ID', 'IdPersona', 'ID_PERSONA') || 900001);
  const firstName = firstEnv('TEST_USER_FIRST_NAME', 'Nombres') || 'Pablo';
  const lastName = firstEnv('TEST_USER_LAST_NAME', 'Apellidos') || 'Arauz Caballero';
  const phone = firstEnv('TEST_USER_PHONE', 'Telefono') || '70000000';
  const userType = firstEnv('TEST_USER_TYPE', 'TipoUsuario') || 'SUPER_ADMIN';
...
async function cleanOfficialUserData(client, testUser = getOfficialUserFromEnv()) {
  ...
  const seedUsernames = [testUser.username, 'pablo.admin'];
  const seedTypes = [testUser.userType, 'SUPER_ADMIN'];
  ...
  await client.query(
    ...
    [
      testUser.idPersona,
      testUser.email,
      seedUsernames.map((value) => value.toLowerCase()),
      seedTypes.map((value) => value.toUpperCase()),
    ],
  );
